        ...
        if status == "failed":
            ...
            if should_emit_terminal_error and not already_notified:
                ...
                if not handled:
                    ...
                    # row, and the web Chat renders only durable ``message.new`` rows —
                    # so persist a terminal notify here too, or an expired Codex login
                    # stops the prompt with no on-screen explanation (Codex P2). The
                    # not-handled branch already persists via ``emit_agent_message``.
                    try:
                        from core.message_mirror import persist_agent_message

                        persist_agent_message(tracked_request.context, "notify", message)
                    except Exception:
                        logger.debug("codex: failed to persist auth-recovery notify", exc_info=True)
