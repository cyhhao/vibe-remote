...
def _pid_matches_service_process(pid: int) -> bool:
    command = get_process_command(pid)
    return bool(command and _command_references_path(command, get_service_main_path()))


def _linux_process_start_time(pid: int) -> float | None:
    proc_stat = Path(f"/proc/{pid}/stat")
    system_stat = Path("/proc/stat")
    if not proc_stat.exists() or not system_stat.exists():
        return None
    try:
        boot_time = None
        ...
        after_command = stat_text.rsplit(")", 1)[1].strip().split()
        start_ticks = int(after_command[19])
        ticks_per_second = os.sysconf(os.sysconf_names["SC_CLK_TCK"])
        return boot_time + (start_ticks / ticks_per_second)
    except (OSError, IndexError, KeyError, ValueError):
        logger.debug("Failed to read Linux process start time for pid=%s", pid, exc_info=True)
        return None
...
def _ps_process_start_time(pid: int) -> float | None:
    try:
        ...
        logger.debug("Failed to invoke ps for process start time pid=%s", pid, exc_info=True)
        return None
...
def get_process_start_time(pid: int) -> float | None:
    if not isinstance(pid, int) or pid <= 0:
        return None
    if os.name == "posix":
        return _linux_process_start_time(pid) or _ps_process_start_time(pid)
    return None


def _current_package_mtime() -> float | None:
    # Only use this in installed-package mode. In a source checkout, editing
    # files while a dev service is running should not make bare `vibe` restart.
    if (get_project_root() / "main.py").exists():
        return None

    markers = [
        get_package_root() / "__init__.py",
        get_service_main_path(),
        get_package_root() / "ui_server.py",
    ]
    mtimes: list[float] = []
...
def _process_predates_current_package(pid: int) -> bool:
    package_mtime = _current_package_mtime()
    process_start_time = get_process_start_time(pid)
    if package_mtime is None or process_start_time is None:
        return False
    return process_start_time + 1.0 < package_mtime


def runtime_processes_stale_after_package_update() -> bool:
    service_pid_path = paths.get_runtime_pid_path()
    if service_pid_path.exists():
        try:
            service_pid = int(service_pid_path.read_text(encoding="utf-8").strip())
        except (OSError, ValueError):
            service_pid = 0
        if (
            service_pid
            and pid_alive(service_pid)
            and _pid_matches_service_process(service_pid)
            and _process_predates_current_package(service_pid)
        ):
            return True
    ...
    if ui_pid_path.exists():
        ...
        if (
            ui_pid
            and pid_alive(ui_pid)
            and _pid_matches_ui_server(ui_pid)
            and _process_predates_current_package(ui_pid)
        ):
            return True
