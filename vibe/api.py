...
def setup_opencode_permission() -> dict:
    """Setup OpenCode permission config by adding 'permission: allow' to config file.
    ...
    try:
        ...
        if config_path.exists():
            try:
                ...
                if not isinstance(config, dict):
                    return {
                        "ok": False,
                        "message": "Config file exists but is not a JSON object",
                        "config_path": str(config_path),
                    }
            except json.JSONDecodeError as e:
                return {
                    "ok": False,
                    "message": f"Config file exists but contains invalid JSON: {e}",
                    "config_path": str(config_path),
                }
        else:
            config = {}

        # Check if permission is already set to allow
        if config.get("permission") == "allow":
            return {
                "ok": True,
                "message": "Permission already set to 'allow'",
                "config_path": str(config_path),
            }
        ...
        return {
            "ok": True,
            "message": "Permission set to 'allow' successfully",
            "config_path": str(config_path),
        }
    except Exception as e:
        logger.error(f"Failed to setup OpenCode permission: {e}", exc_info=True)
