...
# easy to find in `vibe session list`.
_SESSION_TITLE_NUDGE = """\
This session's title is currently {title_state}. If you can tell what this conversation is about, give it a short, descriptive title (about 10 characters or fewer) so it is easy to find later:
...
def _build_session_end_prompt(context: MessageContext) -> str:
    ...
    if looked_up is not None:
        ...
        # Nudge only when the user hasn't named it: empty title, or a title_source
        # that isn't "user" (auto-generated by the backend, or not yet filled).
        if not title or title_source != "user":
            title_state = (
                "empty (not set yet)"
                if not title
                else f'"{title}" (auto-generated, not chosen by the user)'
            )
