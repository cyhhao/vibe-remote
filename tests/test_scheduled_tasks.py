...
def test_execute_request_drives_sidebar_dot_for_avibe_scheduled_turn(tmp_path: Path, monkeypatch) -> None:
    # A scheduled avibe turn doesn't go through internal_server._run_turn, so
    # _execute_request must flip the workbench sidebar dot itself: green at start,
    # idle on a clean settle (red if the turn failed). (Codex P2.)
    import core.scheduled_tasks as st

    target = ParsedSessionKey(platform="avibe", scope_type="project", scope_id="proj_x")
    monkeypatch.setattr(
        st,
        "resolve_session_id_target",
        lambda sid: st.ResolvedSessionIdTarget(
            session_id=sid,
            session_key=target,
    ...
    async def _handle(context, message, parsed_session_key=None):
        return None  # clean success

    controller = SimpleNamespace(
        platform_settings_managers={},
        im_clients={"avibe": SimpleNamespace()},
        get_im_client_for_context=lambda _c: SimpleNamespace(
            should_use_thread_for_reply=lambda: True,
            should_use_thread_for_dm_session=lambda: False,
        ),
        mark_turn_running=lambda sid: dot.append(("running", sid)),
        pop_turn_failed=lambda sid: False,
        set_agent_status=lambda sid, status: dot.append((status, sid)),
        message_handler=SimpleNamespace(handle_scheduled_message=_handle),
    )
    service = ScheduledTaskService(controller=controller, store=ScheduledTaskStore(tmp_path / "s.json"))
    ...
    # Green at start, then idle once the (successful) turn settles.
    assert dot == [("running", "ses-abc"), ("idle", "ses-abc")]
